...
class Triangle(object):
    ...
    def area(self):
        ...
        #        They include the Example in the above doc-string.
        # ---------------------------------------------------------------------
        lengtha = math.sqrt((self.a.x-self.b.x)**2 + (self.a.y-self.b.y)**2)
        lengthb = math.sqrt((self.b.x-self.c.x)**2 + (self.b.y-self.c.y)**2)
        lengthc = math.sqrt((self.c.x-self.a.x)**2 + (self.c.y-self.a.y)**2)
        s = 0.5 *(lengtha + lengthb + lengthc)
        a = math.sqrt(s*(s-lengtha)*(s-lengthb)*(s-lengthc))
        if a > self.largest_area:
            self.largest_area = a
        return a
